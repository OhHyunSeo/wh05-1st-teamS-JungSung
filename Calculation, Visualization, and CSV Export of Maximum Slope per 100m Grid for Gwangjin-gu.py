...
dem_path = './37705.img'             # 성동구 2022 DEM 자료 (파일 이름 예시)
...
with rasterio.open(dem_path) as src:
    ...
    crs = src.crs
    resolution = src.res[0]  # 픽셀 크기 (x방향; y도 동일하다고 가정)
...
# 3. 100m 격자별 최대 경사도 계산 (Zonal Statistics)
grid = gpd.read_file(grid_shp_path)  # 최초에 읽은 grid 객체 사용
...
print(f"전체 격자 데이터가 '{csv_all}' 파일로 저장되었습니다.")

# 10. 상위 50% 데이터 (경사도가 높은 순으로 상위 절반) 저장
n_total = len(grid_df)
...
print(f"상위 50% 격자 데이터가 '{csv_top}' 파일로 저장되었습니다.")
